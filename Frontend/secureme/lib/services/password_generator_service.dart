...
class PasswordGeneratorService {
  ...
  static String buildAlphabet(
      Set<CharClass> classes, bool excludeAmbiguous, String? allowedSet) {
    String alphabet = '';
    ...
    if (alphabet.length < 10) {
      throw ArgumentError(
          'Alphabet too small (${alphabet.length} chars). Need at least 10.');
    }
  ...
  static void satisfyMustIncludeInPlace(List<String> pwdChars,
      PasswordPolicy policy, Map<CharClass, String> alphByClass) {
  ...
  static String randomPassword(RandomCharsOptions opts) {
    final alphabet = buildAlphabet(
        opts.classes, opts.excludeAmbiguous, opts.policy?.allowedSet);
    ...
    if (opts.policy?.mustInclude != null &&
        !mustIncludeSatisfied(pwdChars.join(''), opts.policy)) {
      final alphByClass = <CharClass, String>{
        CharClass.upper:
            buildAlphabet({CharClass.upper}, false, opts.policy?.allowedSet),
        CharClass.lower:
            buildAlphabet({CharClass.lower}, false, opts.policy?.allowedSet),
        CharClass.digit:
            buildAlphabet({CharClass.digit}, false, opts.policy?.allowedSet),
        CharClass.symbol:
            buildAlphabet({CharClass.symbol}, false, opts.policy?.allowedSet),
      };
